...
class DirectionalStatistics:
    ...
    def mean_cartesian(self) -> tuple[float, float]:
        ...
        """
        return _cartesian_mean(self.cartesian_data)
    ...
    def var(self) -> float:
        ...
        """
        return _cartesian_variance(self.cartesian_data)
    ...
    def normalize_format(self) -> None:
        ...
        if self.format == Directional_Format.cartesian:
            ...
            self.polar_data         = _cartesian_to_polar(self.cartesian_data, 2 * np.pi)

        elif self.format == Directional_Format.polar:
            self.polar_data         = self.raw_data
            self.cartesian_data     = _polar_to_cartesian(self.polar_data, 2 * np.pi)
    ...
    @staticmethod
    def convert(data:           npt.ArrayLike,
                input_format:   Union[Directional_Format, str],
                output_format:  Union[Directional_Format, str],
                base:           Union[float, int] = 2 * np.pi) -> npt.ArrayLike:
    ...
    def cartesian_to_polar(cartesian_data:npt.ArrayLike, base:Union[float, int] = 2 * np.pi) -> np.ndarray:
        ...
        """
        return _cartesian_to_polar(cartesian_data, base)
    ...
    def polar_to_cartesian(polar_data:npt.ArrayLike, base:Union[float, int] = 2 * np.pi) -> np.ndarray:
        ...
        """
        return _polar_to_cartesian(polar_data, base)
    ...
    def project_to_unit_circle(x:npt.ArrayLike, y:npt.ArrayLike) -> np.ndarray:
        ...
        Returns:
            np.ndarray: Array-like object containing cartesian data.
        """
        return _project_to_unit_circle(x, y)
    ...
    def distance_between_angles(theta1:npt.ArrayLike, theta2:npt.ArrayLike, base:Union[float, int] = 2 * np.pi) -> np.ndarray:
        ...
        """
        return _distance_between_angles(theta1, theta2) * base / (2 * np.pi)
    
    @staticmethod
    def cartesian_mean(data:        npt.ArrayLike,
                       input_format:Union[Directional_Format, str],
                       base:        Union[float, int] = 2 * np.pi) -> tuple[float, float]:
        ...
        return _cartesian_mean(cartesian_data)
...
def _cartesian_to_polar(cartesian_data:np.ndarray, base:float = 2 * np.pi) -> np.ndarray:

    base_to_radians = base / (2 * np.pi)
    return_array    = np.zeros(cartesian_data.shape)
    for i in range(cartesian_data.shape[0]):
        x                   = cartesian_data[i, 0]
        y                   = cartesian_data[i, 1]
        r                   = np.sqrt(x**2 + y**2)
        theta               = np.arctan2(y, x) * base_to_radians
        return_array[i, 0]  = r
        return_array[i, 1]  = theta
    
    return return_array

@nb.jit(nb.float64[:, :](nb.float64[:, :], nb.float64), nopython=True)
def _polar_to_cartesian(polar_data:np.ndarray, base:float = 2 * np.pi) -> np.ndarray:
    
        radians_to_base = (2 * np.pi) / base
        return_array    = np.zeros(polar_data.shape)
        for i in range(polar_data.shape[0]):
            r                   = polar_data[i, 0]
            theta               = polar_data[i, 1] * radians_to_base
            x                   = r * np.cos(theta)
            y                   = r * np.sin(theta)
            return_array[i, 0]  = x
            return_array[i, 1]  = y
...
@nb.jit(nb.float64[:, :](nb.float64[:], nb.float64[:]), nopython=True)
def _project_to_unit_circle(x:np.ndarray, y:np.ndarray) -> np.ndarray:
    n               = len(x)
    return_array    = np.zeros((n, 2))
    for i in range(n):
        r                   = np.sqrt(x[i]**2 + y[i]**2)
        return_array[i, 0]  = x[i] / r
        return_array[i, 1]  = y[i] / r
    
    return return_array

@nb.jit(nb.float64[:](nb.float64[:, :]), nopython=True)
def _cartesian_mean(data:np.ndarray) -> np.ndarray:
    return_array        = np.zeros(2)
    projected           = _project_to_unit_circle(data[:, 0], data[:, 1])
    return_array[0]     = np.mean(projected[:, 0])
    return_array[1]     = np.mean(projected[:, 1])
    return return_array

@nb.jit(nb.float64(nb.float64[:, :]), nopython=True)
def _cartesian_variance(data:np.ndarray) -> np.ndarray:
    cartesian_mean = _cartesian_mean(data)
    r              = np.sqrt(cartesian_mean[0]**2 + cartesian_mean[1]**2)
    r              = min(1, r)
    return 1 - r

@nb.jit(nb.float64[:](nb.float64[:], nb.float64[:]), nopython=True)
def _distance_between_angles(theta1:np.ndarray, theta2:np.ndarray):
    n               = len(theta1)
...
@nb.jit(nb.float64[:, :](nb.float64[:, :], nb.float64[:]), nopython=True)
def _generate_cartesian_from_polar(polar_data:np.ndarray, center:np.ndarray) -> np.ndarray:
    n               = polar_data.shape[0]
    return_array    = np.zeros((n, 2))
    for i in range(n):
        r                   = polar_data[i, 0]
        theta               = polar_data[i, 1]
        x                   = r * np.cos(theta)
        y                   = r * np.sin(theta)
        return_array[i, 0]  = x + center[0]
        return_array[i, 1]  = y + center[1]
    
    return return_array

@nb.jit(nb.float64[:, :](nb.float64[:, :], nb.float64[:]), nopython=True)
def _generate_polar_from_cartesian(cartesian_data:np.ndarray, center:np.ndarray) -> np.ndarray:
    n               = cartesian_data.shape[0]
    return_array    = np.zeros((n, 2))
    for i in range(n):
        x                   = cartesian_data[i, 0] - center[0]
        y                   = cartesian_data[i, 1] - center[1]
        r                   = np.sqrt(x**2 + y**2)
